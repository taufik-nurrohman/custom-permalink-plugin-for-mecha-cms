...
Route::accept($config->index->slug . '/(:num)/(:num)/(:num)/(:any)' . $cp_config['extension'], function($year = "", $month = "", $day = "", $slug = "") use($config) {
...
function do_custom_permalink($url) {
    ...
    if($path = Get::articlePath(File::B($url))) {
        ...
        $time = explode('-', $time);
        return $config->url . '/' . $config->index->slug . '/' . $time[0] . '/' . $time[1] . '/' . $time[2] . '/' . $slug . $cp_config['extension'];
    }
    return $url;
}

// fix page types
if(Route::is($config->index->slug . '/(:num)/(:num)/(:num)/(:any)' . $cp_config['extension'])) {
