...
Route::accept($config->index->slug . '/(:num)/(:any)' . $cp_config['extension'], function($year = "", $slug = "") use($config) {
...
function do_custom_permalink($url) {
    ...
    if($path = Get::articlePath(File::B($url))) {
        ...
        $time = explode('-', $time);
        return $config->url . '/' . $config->index->slug . '/' . $time[0] . '/' . $slug . $cp_config['extension'];
    }
    return $url;
}
