...
Route::accept('(:num)/(:num)/(:any)' . $cp_config['extension'], function($year = "", $month = "", $slug = "") use($config) {
...
function do_custom_permalink($url) {
    ...
    if($path = Get::articlePath(File::B($url))) {
        ...
        $time = explode('-', $time);
        return $config->url . '/' . $time[0] . '/' . $time[1] . '/' . $slug . $cp_config['extension'];
    }
    return $url;
}

// fix page types
if(Route::is('(:num)/(:num)/(:any)' . $cp_config['extension'])) {
