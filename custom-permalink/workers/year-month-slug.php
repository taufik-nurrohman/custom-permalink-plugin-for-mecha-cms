...
// pattern: `http://localhost/2015/04/slug`
Route::accept('(:num)/(:num)/(:any)' . $c_cp->extension, function($year = "", $month = "", $slug = "") use($config) {
    if($path = Get::articlePath($slug)) {
        $s = explode('_', File::N($path));
        $s = explode('-', $s[0]);
        if(File::D($config->url_path) !== $s[0] . '/' . $s[1]) {
            Shield::abort('404-article');
        }
    } else {
        Shield::abort('404-article');
...
// to: `http://localhost/2015/04/slug`
function do_cp($url) {
    global $config, $c_cp;
    if($path = Get::articlePath(File::B($url))) {
        list($t, $k, $s) = explode('_', File::N($path), 3);
        $t = explode('-', $t);
        return $config->url . '/' . $t[0] . '/' . $t[1] . '/' . $s . $c_cp->extension;
    }
    return $url;
}

// fix page types
if(Route::is('(:num)/(:num)/(:any)' . $c_cp->extension)) {
